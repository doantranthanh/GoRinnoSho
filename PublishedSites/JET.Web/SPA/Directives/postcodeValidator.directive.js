(function(app) {
    ...
    function postcodeValidator() {
        ...
        var postCodeOutWardRegexp = /^[A-Za-z]{1,2}[0-9Rr][0-9A-Za-z]? {0,1}$/;
            ...
            link: function(scope, element, attributes, ngModel) {
                ngModel.$validators.postcodeValidator = function(modelValue) {
                    if (modelValue.length !== 0) {
                        if (modelValue.length >= 2 && modelValue.length <= 4) {
                            if (postCodeOutWardRegexp.test(modelValue)) {
                                return true;
                            } else {
                                return false;
                            }
                        }
                        if (modelValue.length > 4 && modelValue.length <= 8) {
